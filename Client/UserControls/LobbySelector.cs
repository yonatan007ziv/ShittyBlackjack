...
public partial class LobbySelector : UserControl
{
	...
	internal LobbySelector(TcpClientHandler tcpClientHandler)
	{
		...
		makeLobbyButton.Click += (s, e) => OnCreateLobbyButton();
	...
	private void UpdateLobbyPanel(string lobbiesRepresentation)
	{
		...
		else
		{
			...
			for (int i = 0; i < lobbiesCount; i++)
			{
				...
				string lobbyHostName = currentLobbyParts[2];
				int maxPlayerCount = int.Parse(currentLobbyParts[3]);
				int playerCount = int.Parse(currentLobbyParts[4]);

				Button btn = new Button();
				btn.Text = $"{lobbyName} ({playerCount}/{maxPlayerCount})\nHost: {lobbyHostName}";
	...
	private void OnCreateLobbyButton()
	{
		if (nameField.Text == "" || !int.TryParse(maxPlayersField.Text, out int maxPlayersCount) || maxPlayersCount < 2)
		{
			MessageBox.Show("Invalid lobby");
			return;
		}

		_ = tcpClientHandler.WriteMessage($"CreateLobby:{nameField.Text},{maxPlayersField.Text}");
	}
}
